<?php
/*
Plugin Name: Orto Bundle Selector (English)
...
function gck_get_bundle_offers( $product_id = null ) : array {
    ...
    foreach ( $rows as $row ) {
        ...
        $saving_text   = "Total savings " . number_format($saving_amount, 2, '.', '') . "€";
...
function gck_render_bundle_selector() {
    ...
    <style>
      ...
      .color-darkblue { background: #2a3262; }
  ...
  margin-right:6px;
    ...
    <div class="gck-benefits-box">
        <ul class="gck-benefits-list">
            <?php if ( !has_term( array( 'orto-bokserice', 'orto-bokserice2', 'starter-paketi' ), 'product_cat', $product_id ) ) : ?>
                <li><span class="gck-check">✔</span> <strong>Perfect fit</strong></li>
            <?php endif; ?>

            <li><strong>✔ Premium materials, precise cut</strong></li>
            <li><strong>✔ Comfort without compromise</strong></li>

            <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
                <li style="color: #c00;"><strong>✔ Starter pack available only once per person</strong></li>
                <li style="color: #c00;"><strong>✔ Limited to 1,000 packs</strong></li>
            <?php endif; ?>
        </ul>
    ...
    <div class="gck-top-banner-wrap">
        <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
           ...
           <span>Get your exclusive<br/> starter pack</span>
        </div>
        <?php endif; ?>
    </div>
    ...
    <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
        <div class="dev-banner" data-total="1000" data-sold="354">
            <div class="dev-banner__text">
                ...
                <span>Limited quantity <b class="sold">0</b> / <b class="total">0</b> (Only <b class="remain">0</b> left)</span>
            </div>
        ...
        ?>
            <label style="position: relative; <?php if ( ($loop_index == 1 ||  $loop_index == 3) && ! $show_group_titles) : ?> margin-top: 25px;  <?php endif; ?>"
                ...
                <?php if ( ! $show_group_titles ) : ?>
                    <?php if ( $loop_index == 1 ) : ?>
                        <div class="gck-popular-badge">Most popular 🔥</div>
                    <?php endif; ?>

                    <?php if ( $loop_index == 3 ) : ?>
                        <div class="gck-popular-badge-2">Best price 🔥</div>
                    <?php endif; ?>
                <?php endif; ?>
    ...
    && !has_term( array( 'starter-paketi' ), 'product_cat', $product_id ) 
                ...
                — <span class="bundle-option-title"><?php echo number_format( (float) $data['per'], 2 ); ?>€ / pc</span>
                <?php endif; ?>
...
<div class="bundle-total-line">
    ...
    <span class="line-total"><?php echo number_format( (float) $data['total'], 2 ); ?>€</span>
                ...
                <div class="bundle-pairs <?php echo $is_default ? '' : 'hidden'; ?>"
                    ...
                    <small style="display: block; line-height: 1;"><?php esc_html_e( 'We offer 30 days for a refund or free product exchange – worry-free shopping!', 'gift-card-kompetentnost' ); ?></small>
                </div>
            </label>
        <?php
        $loop_index++;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!valid) {
            e.preventDefault();
            alert('Please select a color and size for each set.');
        }
    });
