...
function gck_get_bundle_offers( $product_id = null ) : array {
    ...
    foreach ( $rows as $row ) {
        ...
        $saving_text   = "Kokkuhoid kokku " . number_format($saving_amount, 2, '.', '') . "€";
...
function gck_split_attrs_color_size( array $custom_attrs ) : array {
    ...
    foreach ( $custom_attrs as $key => $attr ) {
        ...
        $is_color = ( strpos($hay, 'boja') !== false || strpos($hay, 'color') !== false || strpos($hay, 'colour') !== false || strpos($hay, 'värv') !== false );
        $is_size  = ( strpos($hay, 'vel')  !== false || strpos($hay, 'veli') !== false || strpos($hay, 'size') !== false || strpos($hay, 'suurus') !== false );
...
function gck_attr_group_token( string $label, string $type ) : string {
    ...
    if ( $type === 'color' ) {
        $l = str_replace(['boja', 'color', 'colour', 'värv', ':'], ' ', $l);
    } else {
        $l = str_replace(['veličina', 'velicina', 'size', 'suurus', ':'], ' ', $l);
    }
...
function gck_render_bundle_selector() {
    ...
    <style>
      ...
      .color-must { background: #000; } .color-black { background: #000; }
      .color-sinine { background: #203240; } .color-blue { background: #203240; }
      .color-roheline { background: #294d3b; } .color-green { background: #294d3b; }
      .color-hall { background: #706d78; } .color-gray { background: #706d78; }
      .color-punane { background: #ba212f; } .color-red { background: #ba212f; }
      .color-valge { background: #fff; border: 1px solid #ccc; } .color-white { background: #fff; border: 1px solid #ccc; }
      .color-beez { background: #e4e0cf; } .color-beige { background: #e4e0cf; }
      .color-pruun { background: #9f6f4e; } .color-brown { background: #9f6f4e; }
      .color-tumesinine { background: #2a3262; } .color-navy { background: #2a3262; }
  ...
  margin-right:6px;
    ...
    ?>
        <style>
            .price ins { font-size: 25px !important; font-weight: bold; }
            .price del { font-size: 25px !important; }
            .price-badge { font-size: 17px !important; margin-top: -7px !important; }
            .gck-divider span { font-size: 17px; font-weight: 600; color: black; padding: 0 12px; text-transform: uppercase; letter-spacing: -0.2px; text-align: center !important; line-height: 1.3; }
          
           @media (max-width: 991px) {
               .gck-divider span { font-size: 17px; font-weight: 600; color: black; padding: 0 12px; text-transform: uppercase; letter-spacing: -0.2px; text-align: center !important; line-height: 1.3; }
               #title-buy-now  { font-size: 35px !important; letter-spacing: 0.1px !important; }
           }
    ...
    <div class="gck-benefits-box">
        <ul class="gck-benefits-list">
            <?php if ( !has_term( array( 'orto-bokserice', 'orto-bokserice2', 'starter-paketi' ), 'product_cat', $product_id ) ) : ?>
                <li><span class="gck-check">✔</span> <strong>Täiuslik istuvus</strong></li>
            <?php endif; ?>

            <li><strong>✔ Kvaliteetsed materjalid, täpne lõige</strong></li>
            <li><strong>✔ Mugavus ilma kompromissideta</strong></li>

            <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
                <li style="color: #c00;"><strong>✔ Stardipakk saadaval ainult üks kord inimese kohta</strong></li>
                <li style="color: #c00;"><strong>✔ Piiratud 1000 pakini</strong></li>
            <?php endif; ?>
        </ul>
    ...
    <div class="gck-top-banner-wrap">
        <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
           ...
           <span>Hankige oma eksklusiivne<br/> stardipakk</span>
        </div>
        <?php endif; ?>
    </div>
    ...
    <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
        <div class="dev-banner" data-total="1000" data-sold="354">
            <div class="dev-banner__text">
                ...
                <span>Piiratud kogus <b class="sold">0</b> / <b class="total">0</b> (Veel <b class="remain">0</b> tk)</span>
            </div>
        ...
        ?>
            <label style="position: relative; <?php if ( ($loop_index == 1 ||  $loop_index == 3) && ! $show_group_titles) : ?> margin-top: 25px;  <?php endif; ?>"
                ...
                <?php if ( ! $show_group_titles ) : ?>
                    <?php if ( $loop_index == 1 ) : ?>
                        <div class="gck-popular-badge">Kõige populaarsem 🔥</div>
                    <?php endif; ?>

                    <?php if ( $loop_index == 3 ) : ?>
                        <div class="gck-popular-badge-2">Parim hind 🔥</div>
                    <?php endif; ?>
                <?php endif; ?>
                ...
                — <span class="bundle-option-title"><?php echo number_format( (float) $data['per'], 2 ); ?>€ / tk</span>
                <?php endif; ?>

                <br/>

<div class="bundle-total-line">
    <span style="font-weight:normal;">Kokku:</span>
    ...
    <span class="line-total"><?php echo number_format( (float) $data['total'], 2 ); ?>€</span>
                ...
                <div class="bundle-pairs <?php echo $is_default ? '' : 'hidden'; ?>"
                    ...
                    <small style="display: block; line-height: 1;"><?php esc_html_e( 'Pakume 30-päevast raha tagasi garantiid või tasuta tootevahetust – muretu ostlemine!', 'gift-card-kompetentnost' ); ?></small>
                </div>
            </label>
        <?php
        $loop_index++;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!valid) {
            e.preventDefault();
            alert('Palun valige igale komplektile värv ja suurus.');
        }
    });
